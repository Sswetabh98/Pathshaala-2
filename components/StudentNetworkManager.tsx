...
interface StudentNetworkManagerProps {
    currentUser: User;
    allUsers: User[];
...
const StudentNetworkManager: React.FC<StudentNetworkManagerProps> = (props) => {
    const { subView } = props;
    const pendingRequestCount = useMemo(() => {
        const myProfile = props.currentUser.profile as StudentProfile;
        const incomingCount = myProfile.connectionRequests?.filter(req => req.status === 'pending').length || 0;
        
        let outgoingCount = 0;
        props.allUsers.forEach(user => {
            if (user.role === 'teacher') {
                const theirProfile = user.profile as TeacherProfile;
...
export default StudentNetworkManager;